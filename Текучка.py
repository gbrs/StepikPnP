...
with open('dataset_3380_5.txt', encoding='utf-8') as f:
    for pupil in f.readlines():
        grade, name, height = pupil.strip().split()
        total_values[int(grade)][0] += 1
        total_values[int(grade)][1] += int(height)

for i in range(1, 12):
    if not total_values[i][0]:
        print(i, '-')
    else:
        print(i, total_values[i][1]/total_values[i][0])
